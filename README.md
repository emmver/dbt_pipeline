...
```mermaid
flowchart TD
    subgraph raw["main_raw — Python load"]
        RP[projects]
        RE[employees]
        RT[timesheets]
    end
    subgraph stg["main_staging — typed, deduped, single-source rejects dropped"]
        SP[stg_projects]
        SE[stg_employees]
        ST[stg_timesheets]
    end
    subgraph int["main_intermediate — cross-source; inner-join filters dangling FKs"]
        IV[int_timesheets_validated]
    end
    subgraph mart["main_marts — pure joins/aggregations, contracts enforced"]
        DP[dim_projects]
        DE[dim_employees]
        FT[fct_timesheets]
        AP[agg_hours_per_project]
        AM[agg_hours_per_project_monthly]
        RPE[rpt_project_economics]
    end
    RP --> SP
    RE --> SE
    RT --> ST
...
_10 models: 3 staging + 1 intermediate + 6 marts. Edges are dbt `ref()` dependencies; the intermediate is the only cross-source model (it inner-joins `stg_timesheets` to the two clean staging dims to drop dangling FKs). See the live lineage graph at <https://emmver.github.io/dbt_pipeline/>._
...
- [`dbt/docs/er_diagram.mmd`](dbt/docs/er_diagram.mmd) — Mermaid ER diagram (embedded below; also viewable as the interactive ERD in the [live docs site](https://emmver.github.io/dbt_pipeline/)).
- [`warehouse/schema.sql`](warehouse/schema.sql) — the intended DDL (with PK / FK / CHECK declared for design completeness).
- [`dbt/docs/schema_design.md`](dbt/docs/schema_design.md) — design notes, column types, the full DB-vs-pipeline enforcement table, and assumptions.
